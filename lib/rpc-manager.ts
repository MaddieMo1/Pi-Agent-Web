  ...
  async send(command: Record<string, unknown>): Promise<unknown> {
    ...
    switch (type) {
      ...
      case "fork": {
        ...
        if (!currentSessionFile) throw new Error("持久化会话缺少会话文件");

        const entry = sessionManager.getEntry(entryId);
        if (!entry) throw new Error("用于分叉的 entry ID 无效");
        ...
        if (!entry.parentId) {
          ...
          if (!forkedPath) throw new Error("创建分叉会话失败");
